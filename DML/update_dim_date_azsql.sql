...
-- Script Name 		:	update_dim_date_azsql.sql
...
CREATE PROCEDURE [SCHEMA.update_dim_date_azsql].update_dim_date_azsql
  @p_sequence         integer
...
AS
  ...
  SET @v_count = 0
  SELECT @v_count = 1
  FROM  [TABLEOWNER].[dim_date_azsql]
  ...
  BEGIN
    ...
    INSERT INTO [TABLEOWNER].[dim_date_azsql]
    ( dim_date_key
    , cal_month
    , cal_year
    , fin_month
    ...
    BEGIN
      ROLLBACK
      SET @p_return_msg = 'Failed Insert into table dim_date_azsql'
  ...
  END
    ...
    SET @v_step = 10
    SET @v_no_of_days = 10001 -- number of days into future to generate records
    ...
    --sSET @v_current_date = CONVERT(DATETIME,GETDATE()) -- normally set to sysdate. Start date for intitial load
    ...
    BEGIN
      ...
      SET @v_step = 100
      UPDATE [TABLEOWNER].[dim_date_azsql]
      SET
